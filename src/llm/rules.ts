...
function switchboardSection(cfg: Config, players: number): string {
  ...
  const bandRows = a.bands.map((b, i) => {
    ...
    const perks = [
      ...
      `warden bribes ${b.bribeCosts.join(', ')} per die, down to ${b.bribeFloor} ${b.bribeFloor === 1 ? 'die' : 'dice'}`,
    ].filter(Boolean).join('; ');
...
- SELL (${cfg.actionCost.SELL} action) sells your ENTIRE hold at the market port you are docked in, once per day. ${al ? `THE CO-OP: the home port pays the least per pound, but landing a real day's catch there (see the port list for the poundage) steps a licensed captain in a co-op band +${cfg.alignment.step.coopLanding} lighter. A token landing does not count. If you have heat stars, every SELL is also the warden's check (see THE SWITCHBOARD).` : `THE CO-OP: the home port pays the least per pound, but landing a real day's catch there (see the port list for the poundage) earns reputation — the only repeatable way to raise it. A token landing does not count.`} Price per lb = max(floor, base − (lb already landed at that port today ÷ that port's step, rounded down)) (+ rare bonus for RARE keepers). Every price is a whole number. A sale floods its OWN price: your lobsters are paid one at a time, best first (rare, then heaviest), each at the price the port stands at when it lands, and each one pushes the price down for the next. So a huge hold sells for less per pound than a small one, and the flood stays on that port for the rest of the day, for everyone. Prices recover overnight.
- Everything unsold in the hold (keepers, jumbos, eggers) loses ${cfg.holdDecayLbPerDay} lb per night (to a minimum of 1 lb).
...
${cfg.breeding.mode === 'breeders'
  ? `Each ground has a public BREEDING STOCK track: the berried females still alive there. It STARTS FULL, at the number of eggers in that ground's bag, and it only ever loses one when a captain KEEPS an egger from that ground (or the cheap engine's exhaust kills one). An egger nobody has drawn yet, or one that was notched and released, is still out there breeding. (If a spawn draws a kept egger back out of the trap, she's back in the water and the track gains one.)`
  : `Every berried female you v-notch and release advances her ground's BREEDING STOCK track, which is public and printed on the board.`} After season${draftSeasons.length > 1 ? 's' : ''} ${draftSeasons.join(', ')} — not before the final season, the ocean gets no relief for the last year — each ground's stock SPAWNS: it rolls one lobster die per band of its track and returns that many lobsters from its trap to its bag, drawn AT RANDOM — you do not choose which ones come back, so a ground you stripped of jumbos does not get its jumbos back on demand.
Dice by ${cfg.breeding.mode === 'breeders' ? 'breeders alive' : 'notches'}: ${cfg.breeding.diceByStock.slice().reverse().filter((r) => r.dice > 0).map((r) => `${r.atLeast}+ → ${r.dice}d`).join(', ')}. The die faces are ${cfg.breeding.dieFaces.join('/')}, so a die averages ${(cfg.breeding.dieFaces.reduce((a, b) => a + b, 0) / cfg.breeding.dieFaces.length).toFixed(1)} lobsters, and a blank is a bad year.
Lobsters come back AT RANDOM, drawn blind from that ground's trap — you can see how full a trap is, never what is in it. ${cfg.breeding.emptyTrapGeneric ? `If the trap runs out, the rest come back as generic ${cfg.seeded.weightLb} lb lobsters.` : 'An empty trap returns nothing however many dice it rolls.'} ${cfg.breeding.mode === 'breeders' ? 'The ocean starts with its breeding stock at full strength; every kept egger is a lobster that will never breed, and the recovery dwindles with each one.' : 'Nobody is asked to donate anything: you already paid, by throwing back a ' + cfg.eggerWeightLb + ' lb lobster you could have landed.'}
