...
// Nodes the player can step to, and whether they can still get back to SOME port
// (any dock — you can refuel/shelter at the nearest one). "safe" = round trip in fuel.
export function reachability(state: GameState, playerId: string) {
  ...
  for (const node of Object.keys(state.config.map.nodes)) {
    ...
    const backHome = distanceToNearestPort(state, node) * step;
    result[node] = {
