...
export class LlmCaptain {
  ...
  private eventsSince(state: GameState): { events: string[]; interrupt?: string } {
    ...
    for (const e of all) {
      ...
      else if (e.startsWith(`${me} is escorted home`)) interrupt = `you were BUSTED away from the counter (a warden patrol, or caught red-handed at the star cap): your catch was seized, your day is over, you are back at the start port, and you launch last tomorrow (your pots are still in the water)`;
